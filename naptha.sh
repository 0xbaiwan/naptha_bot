...
function main_menu() {
    while true; do
        clear
        echo "脚本原创大赌社区哈哈哈哈编写，推特 @ferdie_jhovie，免费开源，请勿相信收费"
        echo "本人仅优化了日志处理等逻辑"
        echo "================================================================"
...
function remove_naptha_node() {
    ...
    if ! groups $USER | grep -q docker; then
        ...
        echo "已将当前用户添加到 docker 组，请重新登录以生效"
    fi
...
function install_naptha_node() {
    ...
    # 克隆 Git 仓库
    echo "正在克隆 Git 仓库..."
    if ! git clone https://github.com/NapthaAI/node.git; then
